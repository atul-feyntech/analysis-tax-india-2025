...
def new_tax(income):
    ...
    Assumptions for salaried individuals:
      ...
      - Revised Slabs (on taxable income = income - 75,000):
          • 0 – ₹4,00,000: Nil
          • ₹4,00,001–₹8,00,000: 5%
    ...
    if taxable <= 1200000:
        # With Section 87A rebate, net tax is zero for taxable income <= ₹12,00,000.
        return 0
...
# ============================================================
slab_names = []
savings_per_taxpayer = []      # Tax saved per taxpayer in each slab
num_taxpayers_slab = []        # Number of taxpayers in each slab
aggregate_savings_slab = []    # Aggregate savings (nominal) per slab
...
# Compute real (inflation-adjusted) total savings:
...
# ============================================================
# Assume the weighted average saving per taxpayer (from AY 2024-25) applies as a baseline for earlier years.
...
# ============================================================
# Assume that the extra disposable income (real) is entirely spent, and for every rupee spent, 50 paise is added as value.
value_addition_multiplier = 0.50
gdp_increase_nominal = real_total_savings_AY2425 * value_addition_multiplier

# India's current GDP: ₹296.58 lakh crore (1 lakh crore = 1e12 rupees)
...
# ============================================================
# 6. PLOTS AND SAVE TO JPEG FILES
# ============================================================
...
plt.tight_layout()
plt.savefig("Tax_Savings_by_Slab_AY2425.jpeg", format='jpeg', dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("Aggregate_Savings_Over_Years.jpeg", format='jpeg', dpi=300)
plt.close()
...
plt.title(f"Estimated GDP Increase (50% Value Addition)\n(~{gdp_increase_percentage:.2f}% of current GDP)")
plt.grid(axis='y', alpha=0.5)
plt.tight_layout()
plt.savefig("GDP_Increase_Estimate.jpeg", format='jpeg', dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig("Taxpayer_Distribution_Pie_AY2425.jpeg", format='jpeg', dpi=300)
plt.close()
